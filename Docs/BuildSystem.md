...
| **MetaParser** | `Source/Parser/CMakeLists.txt` | 可执行文件 | LLVM |
| **Examples** | `Examples/CMakeLists.txt` | 多个可执行文件 | LLVM |
...
| 编译器 | 最低版本 | 平台 |
|--------|----------|------|
| MSVC | 15 (Visual Studio 2017) | Windows |
| G++ | 7.0 | Linux |
...
示例程序依赖 MetaRuntime 和 MetaParser，同时也需要 LLVM。
...
**解决方案：** 确保使用支持 C++17 的编译器，并在 CMake 中启用：
```cmake
